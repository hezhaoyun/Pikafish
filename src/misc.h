...
struct std::hash<Pikafish::FixedString<N>> {
    std::size_t operator()(const Pikafish::FixedString<N>& fstr) const noexcept {
        return Pikafish::hash_bytes(fstr.data(), fstr.size());
    }
};
