...
template<>
struct std::hash<Pikafish::Eval::NNUE::EvalFile> {
    std::size_t operator()(const Pikafish::Eval::NNUE::EvalFile& evalFile) const noexcept {
        std::size_t h = 0;
        Pikafish::hash_combine(h, evalFile.defaultName);
        Pikafish::hash_combine(h, evalFile.current);
        Pikafish::hash_combine(h, evalFile.netDescription);
        return h;
    }
};
